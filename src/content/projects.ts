 ...
 *
 * One array. One type. Used by:
 *   - Homepage project grid
 *   - Case study pages
 *   - Multiverse Project Universe
 *   - SEO / sitemap
 *   - Project navigation (prev/next)
 *
 * To add a project: add one entry here. Everything updates automatically.
 */
...
/** Get project by ID (same as slug) */
export function getProjectById(id: string): ProjectData | undefined {
  return PROJECTS.find((p) => p.id === id);
}

/** Get all project slugs (for static paths / sitemap) */
export function getAllProjectSlugs(): readonly string[] {
  return PROJECTS.map((p) => p.slug);
}

/** Get ordered project list (sorted by number) */
export function getOrderedProjects(): readonly ProjectData[] {
  return [...PROJECTS].sort((a, b) => a.number - b.number);
}

/** Get next/prev project */
