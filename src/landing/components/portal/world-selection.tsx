...
import { useRef, useEffect, useCallback, useState, useMemo, forwardRef, type PointerEvent as ReactPointerEvent } from "react";
...
const WORLDS: readonly WorldDef[] = [
  {
    ...
    gradient: "linear-gradient(160deg, rgba(15,23,42,0.95) 0%, rgba(30,58,95,0.6) 50%, rgba(99,102,241,0.15) 100%)",
    glowColor: "rgba(99,102,241,0.25)",
    borderHover: "rgba(99,102,241,0.5)",
    accentColor: "#818cf8",
    accentBorder: "rgba(99,102,241,0.4)",
    previewGradient: "radial-gradient(ellipse 120% 100% at 50% 0%, rgba(99,102,241,0.15) 0%, transparent 60%)",
  ...
  },
  {
    ...
    gradient: "linear-gradient(160deg, rgba(248,250,252,0.1) 0%, rgba(226,232,240,0.06) 50%, rgba(200,210,220,0.1) 100%)",
    glowColor: "rgba(226,232,240,0.18)",
    borderHover: "rgba(226,232,240,0.45)",
    accentColor: "#e2e8f0",
    accentBorder: "rgba(226,232,240,0.35)",
    previewGradient: "radial-gradient(ellipse 120% 100% at 50% 0%, rgba(226,232,240,0.12) 0%, transparent 60%)",
  ...
  },
  {
    ...
    gradient: "linear-gradient(160deg, rgba(88,28,135,0.6) 0%, rgba(236,72,153,0.3) 50%, rgba(6,182,212,0.15) 100%)",
    glowColor: "rgba(236,72,153,0.2)",
    borderHover: "rgba(236,72,153,0.5)",
    accentColor: "#ec4899",
    accentBorder: "rgba(236,72,153,0.4)",
    previewGradient: "radial-gradient(ellipse 120% 100% at 50% 0%, rgba(236,72,153,0.12) 0%, transparent 60%)",
        ...
        {Array.from({ length: 12 }, (_, i) => (
          <div
            ...
            style={{
              ...
              height: i % 3 === 0 ? 3 : 2,
              borderRadius: "50%",
              background: i % 4 === 0 ? "#c7d2fe" : "#a5b4fc",
              boxShadow: `0 0 ${String(i % 3 === 0 ? 6 : 3)}px ${i % 4 === 0 ? "rgba(199,210,254,0.6)" : "rgba(165,180,252,0.3)"}`,
...
function AppleBackground({ active }: { active: boolean }) {
  ...
  return (
    <div
      ...
      style={{
        position: "absolute", inset: 0, pointerEvents: "none",
        background: "radial-gradient(ellipse 65% 55% at 50% 30%, rgba(226,232,240,0.1) 0%, transparent 70%)",
        ...
        {Array.from({ length: 5 }, (_, i) => (
          <div
            ...
            style={{
              ...
              right: 0,
              height: 1,
              background: `linear-gradient(90deg, transparent 0%, ${i % 2 === 0 ? "rgba(236,72,153,0.2)" : "rgba(6,182,212,0.2)"} 50%, transparent 100%)`,
      ...
      {Array.from({ length: 18 }, (_, i) => (
        <div
          ...
          style={{
            ...
            height: i % 3 === 0 ? 2 : 1.5,
            borderRadius: "50%",
            background: `hsla(${String(220 + (i % 3) * 30)}, 60%, 70%, 0.15)`,
            willChange: "transform, opacity",
          }}
        />
      ))}
...
function HeadingShimmer({ text }: { text: string }) {
  ...
  return (
    <span
      ref={ref}
      style={{
        background: "linear-gradient(135deg, #c7d2fe 0%, #a5b4fc 30%, #e0e7ff 50%, #818cf8 70%, #6366f1 100%)",
  ...
  return (
    <div
      ...
      style={{
        ...
        right: 0,
        height: 1,
        background: "linear-gradient(90deg, transparent 0%, rgba(99,102,241,0.08) 20%, rgba(139,92,246,0.12) 40%, rgba(236,72,153,0.08) 60%, rgba(99,102,241,0.08) 80%, transparent 100%)",
        backgroundSize: "200% 100%",
        pointerEvents: "none",
        zIndex: 0,
      }}
...
function WorldCard({
  ...
  const handleMouseLeave = useCallback(() => {
    ...
    if (cardRefInner.current && !reducedMotion) {
      gsap.to(cardRefInner.current, {
        rotateX: 0, rotateY: 0, transformPerspective: 800,
        duration: ANIMATION_DURATIONS.slow, ease: ANIMATION_EASINGS.expoOut,
      });
    }
  ...
  const BG = useMemo(() => {
    switch (world.id) {
      ...
      case "apple": return AppleBackground;
      case "cyberpunk": return CyberpunkBackground;
      default: return AppleBackground;
    }
  }, [world.id]);
  ...
  return (
    <div
      ref={(el) => { cardRef(el); cardRefInner.current = el; }}
      role="button"
      tabIndex={0}
      ...
      className="world-card focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-indigo-400"
      style={{
        position: "relative",
        borderRadius: 24,
        overflow: "hidden",
        ...
        transition: "box-shadow 0.6s cubic-bezier(0.19,1,0.22,1), border-color 0.6s cubic-bezier(0.19,1,0.22,1), transform 0.6s cubic-bezier(0.19,1,0.22,1), filter 0.6s cubic-bezier(0.19,1,0.22,1), opacity 0.6s cubic-bezier(0.19,1,0.22,1)",
        border: `1px solid ${isHovered ? world.borderHover : "rgba(255,255,255,0.06)"}`,
        boxShadow: isHovered
          ? `0 0 80px ${world.glowColor}, 0 30px 60px -15px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.08)`
          : "0 4px 24px -4px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.04)",
    ...
    >
      ...
      <div
        ...
        style={{
          position: "absolute", inset: 0,
          background: "linear-gradient(180deg, rgba(255,255,255,0.06) 0%, transparent 30%, rgba(0,0,0,0.3) 100%)",
      ...
      <div
        ...
        style={{
          position: "absolute", inset: 0,
          boxShadow: isHovered
            ? "inset 0 2px 20px rgba(255,255,255,0.04), inset 0 -4px 30px rgba(0,0,0,0.15)"
            : "inset 0 1px 10px rgba(255,255,255,0.02), inset 0 -2px 20px rgba(0,0,0,0.1)",
          borderRadius: 24,
      ...
      <div
        ...
        style={{
          ...
          width: "90%", height: 160, borderRadius: "50%",
          background: `radial-gradient(circle, ${world.glowColor} 0%, transparent 70%)`,
          filter: "blur(50px)", opacity: isHovered ? 1 : 0.25,
          transition: "opacity 0.8s cubic-bezier(0.19,1,0.22,1)",
          pointerEvents: "none",
        }}
      />
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div
            ...
            style={{
              ...
              background: isHovered ? world.iconBg : "rgba(255,255,255,0.05)",
              backdropFilter: "blur(16px)",
              border: `1px solid ${isHovered ? world.iconBorder : "rgba(255,255,255,0.08)"}`,
              fontSize: "1.375rem",
        ...
        <div>
          <span
            style={{
              ...
              display: "block", marginBottom: "clamp(0.5rem, 1vw, 0.75rem)",
            }}
          >
            {world.subtitle}
          </span>
          ...
          <h3
            style={{
              ...
              color: "#f0f0f5", margin: "0 0 clamp(0.5rem, 1vw, 0.75rem) 0",
          ...
          >
            ...
            >
              <path d="M4 9H14M11 5L15 9L11 13" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
      </div>
...
export function WorldSelection({ visible, onExit }: { visible: boolean; onExit: () => void }) {
    ...
    const ctx = gsap.context(() => {
      ...
      if (subtitleRef.current) {
        tl.fromTo(subtitleRef.current,
          ...
          { y: 0, opacity: 1, filter: "blur(0px)", duration: reducedMotion ? 0 : 0.7, ease: ANIMATION_EASINGS.expoOut },
      ...
      if (headingRef.current) {
        tl.fromTo(headingRef.current,
          ...
          { y: 0, opacity: 1, filter: "blur(0px)", duration: reducedMotion ? 0 : 0.8, ease: ANIMATION_EASINGS.expoOut },
  ...
  const handleGlobalKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      const currentIdx = hoveredId
        ? WORLDS.findIndex((w) => w.id === hoveredId)
        : 0;
  ...
  return (
    ...
    >
      ...
      <div ref={liveRef} aria-live="assertive" aria-atomic="true"
      ...
      <div ref={bgRef} aria-hidden="true" style={{
        ...
        <div data-aurora="true" style={{
          position: "absolute", top: "5%", left: "5%", width: "50vw", height: "50vw",
          maxWidth: 700, maxHeight: 700, borderRadius: "50%",
          background: "radial-gradient(circle, rgba(99,102,241,0.06) 0%, transparent 60%)",
          filter: "blur(100px)", willChange: "transform",
        }} />
        <div data-aurora="true" style={{
          position: "absolute", bottom: "0%", right: "0%", width: "45vw", height: "45vw",
        ...
        <div data-aurora="true" style={{
          ...
          maxWidth: 900, maxHeight: 900, borderRadius: "50%",
      ...
      <button
        ...
        className="portal-back-btn focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-indigo-400"
        style={{
          position: "absolute", top: "clamp(1rem, 3vh, 2rem)", left: "clamp(1.5rem, 5vw, 3rem)",
          display: "flex", alignItems: "center", gap: 8,
          background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)",
          borderRadius: 10, color: "rgba(226,232,240,0.6)",
          fontFamily: "'JetBrains Mono', monospace",
      ...
      <div style={{ textAlign: "center", marginBottom: "clamp(2rem, 5vh, 4rem)" }}>
        <span
          ...
          style={{
            ...
            display: "block", marginBottom: "clamp(0.75rem, 1.5vw, 1rem)",
          }}
        >
      ...
      <div
        ...
        style={{
          ...
          fontFamily: "'JetBrains Mono', monospace",
          fontSize: "0.5625rem", letterSpacing: "0.15em",
          textTransform: "uppercase" as const,
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <kbd style={{
            padding: "2px 6px", borderRadius: 4, fontSize: "0.5rem",
        ...
        <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <kbd style={{
            padding: "2px 8px", borderRadius: 4, fontSize: "0.5rem",
            background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)",
          }}>⏎</kbd>
          Select
        </span>
